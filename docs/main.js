...
window.setInterval(function(){
    ...
    if(Nationalized==true){
        ...
        BaseCapability = Math.log10(AIcapabilities)*10;
        //alignment hurdles.  BaseCapability = 10*log10(EFLOPs); since 1 EFLOP = 10^18, raw FLOPs = 10^(BaseCapability/10 + 18).
        //So BaseCapability=90 (="9.0") == 10^27 FLOPs, 100=10^28, 110=10^29, 120=10^30.
        //Each hurdle permanently subtracts 10 from CEV when first crossed.
        if (BaseCapability > 120){
            Hurdle_mod = -40;
            AlignmentHurdlesNow =
            ...
            + "10<sup>30</sup> FLOPs: -10 from the 'fragility of value' vs extreme optimization pressure<br />";
            AlignmentHurdlesSoon = "";
        } else if (BaseCapability > 110){
            Hurdle_mod = -30;
            AlignmentHurdlesNow =
            "10<sup>27</sup> FLOPs: -10 from AI deception driven by instrumental convergence<br />"
            + "10<sup>28</sup> FLOPs: -10 from AI's situational awareness of the training environment<br />"
        ...
        Skill_Biol_Scale = BaseCapability*2.0 + Skill_Biol_mod;
        Skill_Robo_Scale = BaseCapability*2.5 + Skill_Robo_mod;
        // Skill_Visu here is re-synced in TrainAI; in continuous mode we keep Skill_X === Skill_X_Scale
        if (Continuous_Flag == 1){
            Skill_Visu = Skill_Visu_Scale;
            Skill_Lang = Skill_Lang_Scale;
            Skill_Code = Skill_Code_Scale;
            Skill_Biol = Skill_Biol_Scale;
            Skill_Robo = Skill_Robo_Scale;
        }
        // Media & Persuasion panel uses the *larger* of visual (deepfakes) and language (propaganda):
        Skill_Media_Scale = Math.max(Skill_Visu_Scale, Skill_Lang_Scale);
        document.getElementById("Nat_Visu_Scale").innerHTML = Skill_Media_Scale.toLocaleString(undefined, {minimumFractionDigits: 0, maximumFractionDigits: 0}) + "%";
        ...
        // Researchers generate insights ~10x faster after nationalization so the
        // endgame project budget is meaningful (180 days * 5 ticks/day * 0.002 * 50 researchers = 90 insights).
        // Automated Alignment Researcher, if purchased, bumps this by 1.5x.
        Insights = Insights + 0.002*Researchers*(AlignAutoResearchMult || 1);
        document.getElementById('Insights').innerHTML = Insights.toLocaleString(undefined, {minimumFractionDigits: 1, maximumFractionDigits: 1});


        //calculate risk suppression.  Each domain: remaining_risk = capability_skill - specialist_bonus - defense_projects_bonus
        ...
        Censorship4 = Skill_Media_Scale - Censorship2 - Censorship3;
        ...
        // Endgame systems: CEV/COOP progression, threat events, rival AI race, endings.
        // All of the interesting endgame logic lives in endgameTick() below.
        if (typeof endgameTick === 'function') { endgameTick(); }
